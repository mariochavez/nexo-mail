...
# Each source agent writes its digest fragment into ./sandbox; a merge agent then
# reads those fragments (handed to it in the prompt) and writes the unified
# ./sandbox/inbox-digest.md, which is recorded as the run's artifact.
#
...
class MultiInboxTriage < Nexo::Workflow
  ...
  # A broken availability check must never crash the run — treat a raising check as
  # "available" and let the per-source rescue handle any real failure at run time.
